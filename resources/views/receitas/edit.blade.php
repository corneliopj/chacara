...
@section('title_page', 'Edição de Receita de Venda')
...
<div class="row">
    <div class="col-md-7">
        <div class="card card-success card-outline">
            <div class="card-header">
                <h3 class="card-title">✏️ Editar Venda: {{ $receita->descricao }}</h3>
            </div>
            ...
            <form action="{{ route('receitas.update', $receita) }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" class="form-control @error('data_venda') is-invalid @enderror" id="data_venda" name="data_venda" 
                                value="{{ old('data_venda', Carbon::parse($receita->data_venda)->format('Y-m-d')) }}" required>
                            @error('data_venda')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control @error('cultura_id') is-invalid @enderror" id="cultura_id" name="cultura_id" required>
                                ...
                                @foreach ($culturas as $cultura)
                                    <option value="{{ $cultura->id }}" {{ old('cultura_id', $receita->cultura_id) == $cultura->id ? 'selected' : '' }}>
                                        {{ $cultura->nome }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                         <div class="form-group col-md-4">
                            ...
                            <input type="number" step="0.01" class="form-control @error('quantidade_vendida') is-invalid @enderror" id="quantidade_vendida" name="quantidade_vendida" 
                                value="{{ old('quantidade_vendida', $receita->quantidade_vendida) }}" required min="0.01">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select class="form-control @error('unidade_medida') is-invalid @enderror" id="unidade_medida" name="unidade_medida" required>
                                ...
                                @foreach ($unidades as $unidade)
                                    <option value="{{ $unidade }}" {{ old('unidade_medida', $receita->unidade_medida) == $unidade ? 'selected' : '' }}>{{ $unidade }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('observacoes') is-invalid @enderror" id="observacoes" name="observacoes" rows="2">{{ old('observacoes', $receita->observacoes) }}</textarea>
                        @error('observacoes')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </div>
                ...
                <div class="card-footer">
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save mr-1"></i> Atualizar Receita
                    </button>
    ...
    <div class="col-md-5">
        <div class="callout callout-success mt-4">
            <h5>Informação!</h5>
            <p>
                Manter as informações de venda atualizadas garante a integridade da análise financeira.
            </p>
        </div>
    </div>
</div>
